#[cfg(test)]
mod tests;

use std::{
    collections::HashSet,
    net::{IpAddr, Ipv4Addr, Ipv6Addr, SocketAddr, SocketAddrV4, SocketAddrV6},
    sync::{
        Arc, Mutex, Weak,
        atomic::{AtomicU32, Ordering},
    },
    task::Poll,
};

use bstr::BStr;
use network_interface::{NetworkInterface, NetworkInterfaceConfig};
use parking_lot::RwLock;
use socket2::SockRef;
use tracing::{debug, trace};

use crate::{
    BindOpts, Error, UdpSocket,
    addr::WithScopeId,
    constants::{
        SSDP_MCAST_IPV4, SSDP_MCAST_IPV6_LINK_LOCAL, SSDP_MCAST_IPV6_SITE_LOCAL, SSDP_PORT,
    },
};

/// An IPv6 + IPv4 multicast socket that sends payloads generated by user callbacks to all
/// interfaces.
pub struct MulticastUdpSocket {
    sock: UdpSocket,
    ipv4_addr: SocketAddrV4,
    ipv6_site_local: SocketAddrV6,
    ipv6_link_local: Option<SocketAddrV6>,
    nics: Vec<NetworkInterface>,
}

impl MulticastUdpSocket {
    pub async fn new(
        bind_addr: SocketAddr,
        ipv4_mcast_addr: SocketAddrV4,
        ipv6_site_local_addr: SocketAddrV6,
        ipv6_link_local_addr: Option<SocketAddrV6>,
    ) -> crate::Result<Self> {
        if let Some(ll) = ipv6_link_local_addr {
            if !ipv6_is_link_local_mcast(ll.ip()) {
                return Err(Error::ProvidedLinkLocalAddrIsntLinkLocal);
            }
        }
        if !ipv6_is_site_local_mcast(ipv6_site_local_addr.ip()) {
            return Err(Error::ProvidedSiteLocalAddrIsNotSiteLocal);
        }
        let nics = network_interface::NetworkInterface::show()
            .into_iter()
            .flatten()
            .collect::<Vec<_>>();
        if nics.is_empty() {
            return Err(Error::NoNics);
        }
        let opts = BindOpts {
            request_dualstack: true,
            reuseport: true,
        };
        let sock = UdpSocket::bind_udp(bind_addr.into(), opts)?;
        let sock = Self {
            sock,
            ipv4_addr: ipv4_mcast_addr,
            ipv6_link_local: ipv6_link_local_addr,
            ipv6_site_local: ipv6_site_local_addr,
            nics,
        };
        sock.bind_multicast().await?;
        Ok(sock)
    }

    pub fn nics(&self) -> &[NetworkInterface] {
        &self.nics
    }

    pub async fn recv_from(&self, buf: &mut [u8]) -> std::io::Result<(usize, SocketAddr)> {
        self.sock.recv_from(buf).await
    }

    pub async fn send_to(&self, buf: &[u8], addr: SocketAddr) -> std::io::Result<usize> {
        self.sock.send_to(buf, addr).await
    }

    async fn bind_multicast(&self) -> crate::Result<()> {
        let mut joined = false;
        if self.sock.bind_addr().is_ipv4() {
            joined = try_join_v4(&self.sock, *self.ipv4_addr.ip(), Ipv4Addr::UNSPECIFIED);
        }

        for nic in self.nics.iter() {
            let mut has_link_local = false;
            let mut has_site_local = false;

            for addr in nic.addr.iter() {
                match (addr.ip(), self.sock.bind_addr().is_ipv6()) {
                    (IpAddr::V4(iface_addr), is_ipv6)
                        if iface_addr.is_private() && !iface_addr.is_loopback() =>
                    {
                        if !is_ipv6 {
                            joined |= try_join_v4(&self.sock, *self.ipv4_addr.ip(), iface_addr);
                        } else {
                            joined |= try_join_v6(
                                &self.sock,
                                self.ipv4_addr.ip().to_ipv6_mapped(),
                                nic.index,
                            )
                        }
                    }
                    (IpAddr::V6(addr), true) => {
                        if addr.is_loopback() {
                            continue;
                        }
                        if ipv6_is_link_local(&addr) {
                            has_link_local = true;
                        } else {
                            has_site_local = true;
                        }
                    }
                    _ => continue,
                }
            }

            if has_site_local {
                joined |= try_join_v6(&self.sock, *self.ipv6_site_local.ip(), nic.index);
            }

            if let Some(ll) = self.ipv6_link_local {
                if has_link_local {
                    joined |= try_join_v6(&self.sock, *ll.ip(), nic.index);
                }
            }
        }

        if !joined {
            return Err(Error::MulticastJoinFail);
        }

        self.sock
            .socket()
            .writable()
            .await
            .map_err(Error::Writeable)?;

        Ok(())
    }

    pub async fn send_multicast_msg(
        &self,
        buf: &[u8],
        opts: &MulticastOpts,
    ) -> crate::Result<usize> {
        // This is .poll_fn() so that we call .set_multicast() immediately before sending a packet.
        // If it's repolled it'll get called again just before the send.

        std::future::poll_fn(|cx| {
            let sref = SockRef::from(self.sock.socket());
            if self.sock.bind_addr().is_ipv6() {
                if let Err(e) = sref.set_multicast_if_v6(opts.interface_id) {
                    return Poll::Ready(Err(Error::SetMulticastIpv6(e)));
                }
            } else {
                let ifaddr = match opts.interface_addr {
                    IpAddr::V4(ipv4_addr) => ipv4_addr,
                    IpAddr::V6(_) => {
                        return Poll::Ready(Err(Error::SendMulticastMsgProtocolMismatch));
                    }
                };
                if let Err(e) = sref.set_multicast_if_v4(&ifaddr) {
                    return Poll::Ready(Err(Error::SetMulticastIpv4(e)));
                }
            };

            self.sock
                .poll_send_to(cx, buf, opts.mcast_addr)
                .map_err(Error::Send)
        })
        .await
    }

    pub async fn try_send_mcast_everywhere(
        &self,
        get_payload: &impl Fn(&MulticastOpts) -> Option<String>,
    ) {
        let sent = Mutex::new(HashSet::new());
        let sent = &sent;

        let futs = self
            .nics
            .iter()
            .flat_map(|ni| ni.addr.iter().map(move |a| (ni.index, a.ip())))
            .filter_map(|(ifidx, ifaddr)| {
                let ipv6_link_local = self
                    .ipv6_link_local
                    .filter(|_| matches!(ifaddr, IpAddr::V6(v6) if ipv6_is_link_local(&v6)));
                let mcast_addr: SocketAddr = match (ifaddr, ipv6_link_local) {
                    (IpAddr::V4(a), _) if !a.is_loopback() && a.is_private() => {
                        self.ipv4_addr.into()
                    }
                    (IpAddr::V6(a), Some(mlocal)) if !a.is_loopback() => {
                        mlocal.with_scope_id(ifidx).into()
                    }
                    (IpAddr::V6(a), None) if !a.is_loopback() => {
                        self.ipv6_site_local.with_scope_id(ifidx).into()
                    }
                    _ => {
                        trace!(oif_id=ifidx, addr=%ifaddr, "ignoring address");
                        return None;
                    }
                };
                Some(MulticastOpts {
                    interface_id: ifidx,
                    interface_addr: ifaddr,
                    mcast_addr,
                })
            })
            .filter_map(|opts| {
                let payload = get_payload(&opts)?;
                let fut = async move {
                    if !sent
                        .lock()
                        .unwrap()
                        .insert((payload.clone(), opts.uniq_key()))
                    {
                        trace!(?opts, "not sending duplicate payload");
                        return;
                    }

                    match self.send_multicast_msg(payload.as_bytes(), &opts).await {
                        Ok(sz) => trace!(?opts, size=sz, payload=?payload, "sent"),
                        Err(e) => {
                            debug!(?opts, payload=?payload, "error sending: {e:#}")
                        }
                    };
                };
                Some(fut)
            });

        futures::future::join_all(futs).await;
    }
}

fn try_join_v4(sock: &UdpSocket, addr: Ipv4Addr, iface: Ipv4Addr) -> bool {
    trace!(multiaddr=?addr, interface=?iface, "joining multicast v4 group");
    if let Err(e) = sock.socket().join_multicast_v4(addr, iface) {
        debug!(multiaddr=?addr, interface=?iface, "error joining multicast v4 group: {e:#}");
        return false;
    }
    true
}

fn try_join_v6(sock: &UdpSocket, addr: Ipv6Addr, ifindex: u32) -> bool {
    trace!(multiaddr=?addr, interface=?ifindex, "joining multicast v6 group");
    if let Err(e) = sock.socket().join_multicast_v6(&addr, ifindex) {
        debug!(multiaddr=?addr, interface=?ifindex, "error joining multicast v6 group: {e:#}");
        return false;
    }
    true
}

fn ipv6_is_link_local(ip: &Ipv6Addr) -> bool {
    const LL: Ipv6Addr = Ipv6Addr::new(0xfe80, 0, 0, 0, 0, 0, 0, 0);
    const MASK: Ipv6Addr = Ipv6Addr::new(0xffff, 0xffff, 0xffff, 0xffff, 0, 0, 0, 0);

    ip.to_bits() & MASK.to_bits() == LL.to_bits() & MASK.to_bits()
}

fn ipv6_is_link_local_mcast(ip: &Ipv6Addr) -> bool {
    const LL: Ipv6Addr = Ipv6Addr::new(0xff02, 0, 0, 0, 0, 0, 0, 0);
    const MASK: Ipv6Addr = Ipv6Addr::new(0xff0f, 0xffff, 0xffff, 0xffff, 0, 0, 0, 0);

    ip.to_bits() & MASK.to_bits() == LL.to_bits() & MASK.to_bits()
}

fn ipv6_is_site_local_mcast(ip: &Ipv6Addr) -> bool {
    const LL: Ipv6Addr = Ipv6Addr::new(0xff05, 0, 0, 0, 0, 0, 0, 0);
    const MASK: Ipv6Addr = Ipv6Addr::new(0xff0f, 0xffff, 0xffff, 0xffff, 0, 0, 0, 0);

    ip.to_bits() & MASK.to_bits() == LL.to_bits() & MASK.to_bits()
}

#[derive(Debug, Hash, Clone, Copy, PartialEq, Eq)]
pub struct MulticastOpts {
    pub interface_id: u32,
    pub interface_addr: IpAddr,
    pub mcast_addr: SocketAddr,
}

impl MulticastOpts {
    pub fn iface_ip(&self) -> IpAddr {
        self.interface_addr
    }

    pub fn mcast_addr(&self) -> SocketAddr {
        self.mcast_addr
    }

    fn uniq_key(&self) -> (Option<u32>, Option<Ipv4Addr>, SocketAddr) {
        todo!()
    }
}

pub type HandlerReply = (String, SocketAddr);
pub type HandlerFn = dyn Fn(&[u8], SocketAddr, &mut Vec<HandlerReply>) + Send + Sync + 'static;
pub type Handler = Box<HandlerFn>;

/// A multicast socket with shared .recv_from(). It'll call all subscribed handlers.
/// You MUST spawn task_listen_forever().
pub struct SharedMulticastUdpSocket {
    sock: MulticastUdpSocket,
    handlers: RwLock<Vec<(HandlerId, Handler)>>,
}

type HandlerId = u32;
static NEXT_HANDLER_ID: AtomicU32 = AtomicU32::new(0);

#[derive(Default)]
pub struct HandlerDropGuard {
    id: Option<HandlerId>,
    sock: Weak<SharedMulticastUdpSocket>,
}

impl HandlerDropGuard {
    pub fn disarm(&mut self) {
        self.id = None;
    }
}

impl Drop for HandlerDropGuard {
    fn drop(&mut self) {
        if let Some(hid) = self.id {
            if let Some(sock) = self.sock.upgrade() {
                sock.handlers.write().retain(|(id, _)| *id != hid);
            }
        }
    }
}

impl SharedMulticastUdpSocket {
    pub fn new(sock: MulticastUdpSocket) -> Arc<Self> {
        Arc::new(Self {
            sock,
            handlers: Default::default(),
        })
    }

    pub fn add_handler(self: &Arc<Self>, handler: Handler) -> HandlerDropGuard {
        let id = NEXT_HANDLER_ID.fetch_add(1, Ordering::Relaxed);
        self.handlers.write().push((id, handler));
        HandlerDropGuard {
            id: Some(id),
            sock: Arc::downgrade(self),
        }
    }

    pub async fn send_to(&self, buf: &[u8], addr: SocketAddr) -> std::io::Result<usize> {
        self.sock.send_to(buf, addr).await
    }

    pub fn nics(&self) -> &[NetworkInterface] {
        self.sock.nics()
    }

    pub async fn task_listen_forever(self: Arc<Self>) -> std::io::Result<()> {
        let mut buf = [0u8; 4096];
        let mut replies = Vec::new();
        loop {
            trace!("recv_from poll");
            let (sz, addr) = self.sock.recv_from(&mut buf).await?;

            trace!(data = ?BStr::new(&buf[..sz]), "received");

            for (_id, handler) in self.handlers.read().iter() {
                handler(&buf[..sz], addr, &mut replies);
            }

            for (data, addr) in replies.drain(..) {
                if let Err(e) = self.sock.send_to(data.as_bytes(), addr).await {
                    debug!(msg=?data, ?addr, "error sending reply: {e:#}");
                }
            }
        }
    }

    pub async fn try_send_mcast_everywhere(
        &self,
        get_payload: &impl Fn(&MulticastOpts) -> Option<String>,
    ) {
        self.sock.try_send_mcast_everywhere(get_payload).await
    }
}

pub async fn create_ssdp_socket() -> crate::Result<Arc<SharedMulticastUdpSocket>> {
    let sock = MulticastUdpSocket::new(
        SSDP_PORT,
        SSDP_MCAST_IPV4,
        SSDP_MCAST_IPV6_SITE_LOCAL,
        Some(SSDP_MCAST_IPV6_LINK_LOCAL),
    )
    .await?;
    Ok(SharedMulticastUdpSocket::new(sock))
}
